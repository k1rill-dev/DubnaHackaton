...
class MakeOrder(forms.ModelForm):
    ...
    tg_id = forms.IntegerField(
        label='',
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': "Введите Ваш Telegram ID"})
    )

    is_bonus = forms.BooleanField(widget=forms.CheckboxInput(attrs={'class': 'checkbox-control'}), label='',
                                  required=False)

    class Meta:
        model = Profile
        fields = ['address', 'full_name', 'phone_number', 'tg_id', 'comments']
        widgets = {
            'address': forms.TextInput(attrs={'class': 'field'}),
            'full_name': forms.TextInput(attrs={'class': 'field'}),
            'phone_number': forms.NumberInput(attrs={'class': 'field'}),
